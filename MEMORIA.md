...
| 14/09/2026 | `20260914093000_avaliacao_atendimentos.sql` (aplicada via MCP `apply_migration`, nome no controle: `avaliacao_atendimentos`) | Nova tabela `whatsapp_hub.atendimento_avaliacoes` (avaliação da IA por operadora × conversa × dia: nota, critérios, alertas, métricas objetivas, `ate_mensagem_at` para reavaliar só o que andou), índices por org/dia e por operador/dia, trigger de `updated_at`, **RLS com uma única policy de SELECT** (admin vê a org, operadora vê `operador_id = auth.uid()`; escrita só service role) e cron `wh-avaliar-atendimentos` às 09:00/09:10/09:20 UTC (06h de Itabuna, avalia o dia anterior). Nenhuma tabela existente foi tocada. | **Danilo, 14/09/2026** | `BEGIN; SELECT cron.unschedule('wh-avaliar-atendimentos'); DROP TABLE IF EXISTS whatsapp_hub.atendimento_avaliacoes; COMMIT;` |
| 15/09/2026 | **Não é migração — UPDATE de dado em produção.** `UPDATE whatsapp_hub.conversations SET assigned_to='ab43d249…' (adm.millena@outlook.com), assigned_at='2026-09-15 18:00:00+00' WHERE assigned_to IS NULL` → **268 linhas** (267 abertas + 1 fechada). Depois: 0 conversas sem atendente em 405. | **Danilo, 15/09/2026** ("todas as conversas que não estão atribuídas a ninguém, colocar para milena") | `UPDATE whatsapp_hub.conversations SET assigned_to=NULL, assigned_at=NULL WHERE assigned_to='ab43d249-5c1b-44a8-92c4-c416d6e55e8c' AND assigned_at='2026-09-15 18:00:00+00';` — o carimbo de horário isola exatamente estas 268 |
| 16/09/2026 | `20260916120000_conversa_nasce_com_dona.sql` (aplicada via MCP `apply_migration`, nome no controle: `conversa_nasce_com_dona`) | (1) `app_users.atende_inbox boolean NOT NULL DEFAULT false` — mesma coluna que o rascunho `20260909180000_transferencia_conversa_parada.sql` usa (ADD COLUMN IF NOT EXISTS, as duas convivem). (2) Função `whatsapp_hub._responsavel_ao_nascer()` + trigger `trg_responsavel_ao_nascer` BEFORE INSERT em `conversations`: se nasce sem dona, vai para a atendente fixa do número (`channels.assigned_member`) ou, senão, para quem tem `atende_inbox` e **recebeu menos conversas hoje** (fuso America/Bahia; desempate por quem recebeu há mais tempo). Trava `pg_advisory_xact_lock` para disparos paralelos. Nome começa com `trg_r` para rodar depois de `trg_org_from_parent`. Ninguém marcado → nasce sem dona (não bloqueia). (3) `atende_inbox = true` só para **adm.millena** (admin, coordenadora) e **mangabeiralarah** (operadora). **Cargos não mudaram.** Testada antes em transação desfeita: 6 conversas simuladas (5 em lote + 1 avulsa) alternaram Larah/Millena/Larah/Millena/Larah/Millena, org_id preenchido. | **Danilo, 16/09/2026** ("vamos priorizar atribuir à Millena e Larah" + "pode") | `BEGIN; DROP TRIGGER IF EXISTS trg_responsavel_ao_nascer ON whatsapp_hub.conversations; DROP FUNCTION IF EXISTS whatsapp_hub._responsavel_ao_nascer(); COMMIT;` — a coluna `atende_inbox` pode ficar (a migration de transferência também a usa); conversas já atribuídas continuam com a dona. |
...
| 55 | 🔴 **Conversa fechada NÃO reabre quando o paciente escreve.** O `meta-webhook` só grava a mensagem e nenhum trigger mexe em `status`; a tela abre por padrão só as "Abertas", então a mensagem some da lista. Entre 10 e 15/09, **11 pacientes** escreveram depois do fechamento e **9 ficaram sem resposta** (ex.: Leticia Sousa Silva, 15/09 13:51, "Esses são alguns procedimentos que eu tenho pra fazer"). | Paciente sem resposta | 15/09/2026 |
| 56 | ✅ **RESOLVIDA NA REGRA em 16/09** (migration `conversa_nasce_com_dona`: nasce com Millena ou Larah, alternando pelo recebido no dia). ⚠️ **Acumulado NÃO acertado:** o UPDATE das conversas sem dona de 16/09 (3) e das 10 abertas de Nathaly/Andressa foi **bloqueado pela proteção de permissões** e aguarda o Danilo — comando na entrada de 16/09. Texto original: 🔴 **Conversa nova nasce sem dono.** O canal "WhatsApp Odonto (oficial)" não tem `assigned_member`; o rodízio do trigger `_on_handoff_autoassign` exige três condições que são todas falsas hoje: `app_settings.auto_assign_enabled=false`, `lead_assignment_queue` vazia e nenhuma conta com `is_online=true`. A conversa só ganha dono quando alguém responde. O UPDATE de 15/09 corrigiu o acumulado (268), **não a regra**. | Regra dos 100% atribuídas | 15/09/2026 |
| 57 | 🔴 **Não existe registro de presença.** Nada no frontend nem nas Edge Functions escreve `app_users.is_online` / `last_seen_at` — as 8 contas estão offline e "nunca vistas". Qualquer regra que dependa de "atendente online" (pedido do Danilo em 15/09: espera longa reatribui para quem está online) precisa dessa peça antes. | Reatribuição por espera | 15/09/2026 |
| 58 | **Parcialmente decidido em 16/09:** quem atende = **só Millena e Larah** (Nathaly e Andressa fora; nenhuma das duas enviou mensagem pelo CRM de 08 a 15/09); conversa de disparo **nasce com dona**; critério = **quem recebeu menos no dia**. **Continuam abertas:** tempo de espera para transferir, botão "Em pausa", ninguém online, teto de trocas, rotina de CRM aberto. Texto original — **8 decisões pendentes da regra de distribuição** (Danilo, 15/09): quem entra no rodízio (millena/larah/nathaly/andressa?); rodízio simples × por carga; conversa de disparo já nasce com dona; quanto é "muito tempo" de espera (sugerido 30 min em horário de atendimento); botão "Em pausa"; o que fazer quando ninguém está online; teto de trocas por conversa; e a mudança de rotina (CRM aberto no expediente). | Toda a automação de atribuição | 15/09/2026 |
| 59 | ⏸️ **STAND BY (Danilo, 15/09): liberar operadora a abrir conversa de outra.** Hoje `InboxPage.tsx:86` bloqueia quem não é admin de abrir conversa atribuída a terceiro — só a TELA bloqueia; RLS e `send-operator-*` permitem. Com as 268 na Milena, **Larah e Nathaly não conseguem abrir essas conversas**. A edição chegou a ser feita e foi **revertida sem commit** a pedido do Danilo. | Atendimento em equipe | 15/09/2026 |
| 60 | **Tela Qualidade não publicada.** `src/app/routes/qualidade/QualidadePage.tsx`, `src/hooks/useAvaliacoes.ts` e as entradas de rota/menu estão no commit `e1ed1e3`, mas **não foram para a Vercel**. A Edge Function e o cron já rodam, então a avaliação acontece e ninguém vê pela interface. | Ver a avaliação dos atendimentos | 14/09/2026 |
...
### 2026-09-09 a 16 · configuração · Disparo com recorte por data, prévia do template, avaliação de atendimento por IA e a regra dos 100% atribuídas

- **Pedido:** (1) por que o disparo só oferecia 16 de 24 orçamentos; (2) filtrar o disparo pelos pacientes do lote do dia; (3) prévia do template na hora de montar a campanha; (4) a IA avaliar o atendimento das operadoras; (5) auditoria dos primeiros dias de operação; (6) garantir que nenhuma conversa fique sem dono.
- **Feito:**
  - **Filtro de período do orçamento + trava anti-repetição no disparo** (commit `a313808`, publicado). No modo "Por coluna do funil": *Qualquer data · Hoje · Ontem · Últimos 7 dias · Escolher datas*, lendo `deals.stage_entered_at`; e a caixa "Não enviar para quem já recebeu disparo nos últimos 7 dias", **ligada por padrão**, válida em qualquer modo de público. Motivo: em 09/09 a coluna "Orçamento sob avaliação" alcançava 68 pacientes, dos quais 58 já tinham recebido a campanha da véspera.
  - **Prévia do template no passo 1 do disparo** (`TemplatePreviewCard.tsx`, commit `2be5a12`, publicado): mostra cabeçalho, corpo, rodapé e botões como o paciente lê, com a variável exibindo a origem escolhida, e **avisa quando o texto aprovado tem chaves sobrando** — o defeito `Olá, {{1}}1}}` que foi para 50 pacientes em 08/09.
  - **Avaliação de atendimento por IA** (commit `e1ed1e3`, migration aplicada, Edge Function **v3 publicada**, frontend **não publicado**). Ver pendência #60.
  - **Atribuição das 268 conversas sem dono para adm.millena** (15/09) — ver linha na tabela de mudanças no banco.
- **Régua da avaliação:** critérios tirados do `BASE-CONHECIMENTO-ODONTO.md` (acolhimento §8, entender antes de vender §6, condução do valor §2, próximo passo §9, clareza) + as proibições do CFO (§7) como alerta. Nota e alerta só valem com **trecho literal** da conversa; trecho que não existe no texto é descartado no código. Atendimento com alerta tem a nota geral limitada a 5. Conversa curta demais entra como `insuficiente`, sem chamar a IA.
- **Dois defeitos achados no teste com o dia 11/09 (20 atendimentos):**
  1. **JSON cortado** — com `max_tokens=1400`, 10 das 20 avaliações falharam: o claude-sonnet-5 gasta parte do limite raciocinando. Corrigido para 4000 + uma segunda tentativa com o dobro. Rodada seguinte: 0 erros.
  2. 🔴 **Alertas injustos por régua errada** — a IA marcou 3 atendentes por "preço inventado" ao oferecerem "5x sem juros" / "6x sem juros". A regra é **até** 12x no cartão: 5x e 6x estão dentro. Corrigido (o alerta só vale acima do limite; valor negociado, desconto e bonificação não são alerta) e o dia 11/09 foi reavaliado — os 3 alertas sumiram. **Decisão pendente do Danilo:** se a gestão NÃO autoriza desconto/bonificação, esse caso volta a ser alerta.
- **Auditoria dos primeiros dias (só leitura, 15/09):** início real 07/09 15:58 (1º orçamento), cobertura até 15/09; **9 dias corridos**, não os ~15 supostos. 619 orçamentos, **todos** da importação do WebDental — nenhum criado à mão. Dado de exemplo: 2 funis do template desativados (0 deals), 2 contatos com os números do próprio Danilo, campanha "Teste" e template `teste_conexao`. `deals` **não registra quem cadastrou**, então "por usuário" não é apurável para importação.
- **Conflitos de documentação confirmados:** o `ODONTO.md` descreve 5 etapas começando em "Orçamento apresentado" e canal **só Zernio**; o banco tem **6 etapas** ("Orçamento sob avaliação" + "Paciente marcado") e canal **`provider='meta'`** (Cloud API direta). O `ODONTO.md` também diz que `deals` não tem `stage_entered_at` (tem) e que o tema é escuro (é claro desde 07/09). **O `ODONTO.md` precisa ser atualizado ou explicitamente marcado como histórico.**
- **Arquivos:** `src/components/campaigns/CampaignWizard.tsx` · `src/components/campaigns/TemplatePreviewCard.tsx` (novo) · `src/hooks/useCampaigns.ts` · `src/types/campaigns.ts` · `src/hooks/useAvaliacoes.ts` (novo) · `src/app/routes/qualidade/QualidadePage.tsx` (novo) · `src/app/router.tsx` · `src/app/layout/nav-config.ts` · `supabase/functions/avaliar-atendimentos/index.ts` (nova) · `supabase/migrations/20260914093000_avaliacao_atendimentos.sql` (nova).
- **Banco:** 1 migração (`avaliacao_atendimentos`) + 1 UPDATE de dado (268 conversas). Ambos na tabela de mudanças.
- **Não feito:**
  - **Tela Qualidade não publicada** (#60).
  - **Liberar operadora a abrir conversa de outra:** feito e **revertido sem commit** a pedido do Danilo — stand by (#59).
  - **Regra de atribuição automática:** parada nas 8 decisões (#58), e depende de construir presença online (#57).
  - **Conversa fechada que recebe mensagem** continua sem reabrir (#55) — 4 pacientes esperando desde 14/09.
  - **Templates:** os 6 rascunhos criados (`odonto_orcamento_d1_v2`, `odonto_condicoes_pagamento`, `odonto_orcamento_lembrete_d3`, `odonto_orcamento_ultima_chamada`, `odonto_orcamento_validade`, `odonto_condicao_este_mes`) seguem tratados em outra conversa, por decisão do Danilo. Os 3 aprovados antigos (`_d1`, `_d3`, `_d7`) **têm o defeito `1}}`**.
  - **Correção do `meta-webhook` para o evento `system`** (conversa fantasma, aberta desde 09/09) continua pendente.
- **Próximo:** decidir as 8 da atribuição (#58), publicar a tela Qualidade (#60) e resolver a reabertura de conversa fechada (#55).

### 2026-09-16 · configuração · Conversa nasce com dona (Millena e Larah) e comparativo das atendentes

- **Pedido:** rever a política de atribuição antes da importação do dia; comparar quem trabalha melhor no CRM; atribuir priorizando Millena e Larah. Cargos mantidos: **Millena admin (coordenadora), Larah operadora**.
- **Levantamento (só leitura):**
  - 350 das 407 conversas nasceram de **disparo** (template do sistema), 57 de paciente que escreveu primeiro. A importação só cria orçamento — **quem cria conversa é o disparo**.
  - O rodízio antigo (`_on_handoff_autoassign`) só roda no handoff da IA e exige atribuição ligada + fila + `is_online` — nunca rodou.
  - O canal Odonto está com `ai_enabled=true`: em 16/09 a IA respondeu sozinha 3 conversas novas sem dona (uma às 5h02).
  - `deals.owner_id` está nulo nos 619 orçamentos e `campaigns` não registra quem criou — "quem fechou" não é apurável.
- **Comparativo 08–15/09** (mensagens de operadora, fora notas): Millena 476 msgs · 95 pacientes · 7 dias ativos · mediana 4 min para responder · 88% em até 1h · 78% dos pacientes responderam · 9 aprovados depois do contato (R$ 5.416,75) · nota IA 6,2 (43) · 32 conversas com o paciente por último (25 há +24h, parte fechada — ver #55). Larah 123 msgs · 57 pacientes · 5 dias · 4 min · 88% · 65% · 3 aprovados (R$ 906,79) · nota IA 6,4 (só 9) · 2 esperando. **Nathaly e Andressa: zero mensagens no CRM desde o início.** Critério mais fraco das duas na IA: "entender antes de vender" (5,5 e 5,1). ⚠️ Aprovado "depois do contato" é correlação — a aprovação vem do WebDental.
- **Feito:** migration `20260916120000_conversa_nasce_com_dona.sql` aplicada (ver tabela de mudanças). Testada antes em transação desfeita.
- **Não feito — bloqueado pela proteção de permissões (aguarda o Danilo):** acerto do acumulado. Comando pronto (carimbo `2026-09-16 12:30:00+00` isola as linhas):
  ```sql
  with alvo as (
    select c.id, row_number() over (order by c.last_message_at desc, c.id) rn
    from whatsapp_hub.conversations c left join auth.users u on u.id=c.assigned_to
    where c.status <> 'closed'
      and (c.assigned_to is null or u.email in ('nathalypereiraburgues@gmail.com','andressamcbf@gmail.com'))),
  ...
  update whatsapp_hub.conversations c
     set assigned_to = case when a.rn % 2 = 1 then d.larah else d.millena end,
         assigned_at = '2026-09-16 12:30:00+00'
    from alvo a, dest d where c.id = a.id;
  ```
  Antes do bloqueio eram 3 sem dona (16/09) + 10 abertas de Nathaly/Andressa, todas com a última mensagem enviada pela clínica (nenhum paciente esperando). Reverter exige guardar antes quem era a dona de cada uma (as 10 de Nathaly/Andressa não voltam só pelo carimbo).
- **Recado para a frente que constrói a transferência de conversa parada (`20260909180000`, não aplicada):** (1) a coluna `atende_inbox` **já existe** e já está marcada; (2) o **passo 4 daquela migration marca 4 atendentes — tem de sair ou ser reduzido a Millena e Larah**, senão desfaz a decisão do Danilo; (3) o critério dela ("menos conversas abertas") manda tudo para a Larah enquanto a Millena tiver 310 abertas — considerar contar só movimento recente.
- **Próximo:** Danilo liberar o acerto do acumulado; decidir as abertas da #58; #59 continua travando a Larah de abrir conversa da Millena.

### 2026-09-16 · configuração · Disparo não vai para paciente que só tem orçamento aprovado

- **Pedido (Danilo, 16/09):** "os disparos só devem ser feitos para pacientes que não aprovaram orçamentos". **Decisão B:** quem aprovou um orçamento mas tem **outro ainda não aprovado** (aprovou a limpeza, não o implante) **continua recebendo**. Sai só quem tem aprovado e nenhum outro em aberto.
- **Achado antes da mudança:** nada no disparo olhava aprovação — nem a montagem do público (funil, etiqueta, base toda, campo, lista da importação) nem o envio. 135 pacientes com orçamento aprovado; **43 receberam disparo depois de já terem aprovado**; 23 dos que só têm aprovado já receberam algum disparo.
- **Definições usadas:** *aprovado* = `deals.status='won'` **ou** etapa com `stages.is_won` (a importação cria o aprovado na etapa com status `open` e só depois vira `won`), arquivado incluso. *Em aberto* = não aprovado e não arquivado — inclui "Não aprovado" (recusou), que continua recebendo. Paciente sem orçamento nenhum não é afetado.
- **Feito (código, não publicado):**
  - `src/hooks/useCampaigns.ts`: `removerQuemSoTemAprovado` roda antes da trava anti-repetição em todo caminho (`aplicarTravasDoDisparo`), inclusive a lista pronta da importação. Lê os orçamentos em páginas de 1000 (limite do PostgREST). No modo funil, orçamento aprovado não conta como motivo para entrar: a coluna "Aprovado" alcança 0. `previewAudience` e `createAndQueue` devolvem `removidosAprovado`. ⚠️ O arquivo usa CRLF — manter.
  - `CampaignWizard.tsx`: contagem por coluna ignora aprovado; caixa "Pacientes que vão receber" mostra "X com orçamento aprovado ficaram de fora"; toast também. `ImportOrcamentosDialog.tsx`: aviso no toast.
  - `dispatch-campaign/index.ts` (segunda trava): depois de reservar o lote e antes de enviar, `contatosSoComAprovado` marca a linha `failed` com "Paciente com orçamento aprovado — não recebe disparo" e soma no contador de falhas. **Se a conferência der erro, não envia** — devolve a reserva e tenta no próximo tick. `loadDealData` passa a preferir o orçamento em aberto mais recente para as variáveis do template. Publicado (v2) era idêntico ao repositório antes da mudança.
- **Conferido nos dados reais (mesma regra em SQL):** saem **86** pacientes (só aprovado), ficam **49** (aprovado + outro em aberto). Por coluna, antes → depois: Sob avaliação 337→337 · Aguardando decisão 4→4 · Paciente marcado 9→9 · Não aprovado 8→8 · **Aprovado 135→0**. Única linha pendente na fila (campanha "Teste", pausada) não é afetada. `tsc` e `npm run build` ok; `deno check` com os mesmos 4 erros de tipo que já existiam em `_shared` (nenhum no arquivo alterado). Tela não testada no navegador (exige login).
- **Não coberto:** `check-follow-ups` nos gatilhos `inactivity`/`no_purchase` envia 1:1 sem passar pela fila (0 regras ativas hoje); `repurchase-dispatch` (recompra, desligado). O `no_reply` passa pela fila e já é pego pela trava do envio.
...
- **Publicado** com o "pode" do Danilo (17/09): commit `1a5aecf`, só os 2 arquivos do Painel + registros (o diff de `_shared/llm.ts` e `process-ai-message` é de outra frente e ficou fora). Conferido no pacote servido pela Vercel (`DashboardPage-1Tdd4piS.js`): padrão `this_month` e o texto novo do período. Tela logada não vista (login é do Danilo).
- **Próximo:** Danilo reimportar 01/09 até hoje (caixa de encerrar desmarcada) e recontar o público do disparo.

### 2026-09-17 · configuração · IA passa o contato da área médica

- **Pedido (Danilo, 17/09):** a IA identificar mensagem que é da parte médica e mandar o número de contato. **Números dados pelo Danilo: (73) 3612-4180 e (73) 99833-4180 — "coloque os dois".** A pergunta "depois do número, a conversa vai para a equipe?" ficou sem resposta; assumida a recomendação: **não** (o número resolve).
- **Levantamento (só leitura, 05 a 17/09):** ≥ 20 mensagens da área médica no WhatsApp Odonto (ginecologista, oftalmo, cardiologista, neurologista, otorrino, psicóloga, exame de sangue, resultado/entrega de exame). A IA respondia "aqui é clínica de odontologia, não temos X" e prometia a recepção; um paciente perguntou "Tem o número da área médica?" e esperou **20h**. A equipe já respondia à mão com o (73) 3612-4180 (6 conversas).
- **Achado lateral:** as instruções atuais nunca mandam a IA usar o marcador `[HANDOFF]`, então ela **nunca pausa sozinha** — o "depois disso não responda mais" do prompt não tem efeito mecânico (392 mensagens da IA em 10 dias). Não mexido.
- **Mudança preparada (só texto, em `whatsapp_hub.ai_agent_config.system_prompt`, org `2f61f310…`; prompt atual md5 `625f2a40a80471ef61be711dad9de7a5`, 3371 caracteres):**
  1. título "Os dois únicos fatos que você pode informar" → "Os fatos que você pode informar";
  2. nova linha nos fatos: contato da área médica (73) 3612-4180 e (73) 99833-4180, só no caso "Assunto da área médica";
  3. nova seção "Assunto da área médica — mande o contato certo" antes de "Situações que vão aparecer": lista do que é médico (especialidades, psicólogo/nutri/fisio, laboratório, exame que não é dos dentes, resultado/entrega de exame, pedido do número) · resposta pronta com os dois números · nunca dizer que "não tem" a especialidade · sem valor/dia/horário/nome de médico · sem chamar a recepção nem frase de encerramento · assunto de dente junto = número + recepção da odonto · **na dúvida (exame, tomografia, raio-x, panorâmica, "a doutora", "consulta" sem dizer de quê) perguntar "É sobre a odontologia ou sobre consulta médica?"**.
- **Aplicação:** `UPDATE` com três `replace()` e trava `md5(system_prompt) = '625f2a40…'`. 1ª tentativa negada pela proteção de permissões; **aplicada na 2ª, com o "A" do Danilo, em 17/09 12:49 UTC**. Depois: md5 `64d2d8fa9624151105c3d68788fc4d5f`, 5069 caracteres. Conferido: seção, os dois números e o título novo presentes; a reversão abaixo devolve exatamente o md5 original. Vale na próxima mensagem (a função lê o prompt a cada chamada; nenhum deploy).
- **Reverter** (testado por SELECT: devolve o md5 original):
  ```sql
  update whatsapp_hub.ai_agent_config set system_prompt = replace(replace(replace(system_prompt,
    substring(system_prompt from position('## Assunto da área médica' in system_prompt) for position('## Situações que vão aparecer' in system_prompt) - position('## Assunto da área médica' in system_prompt)), ''),
    E'- Contato da área médica da Amor Saúde: (73) 3612-4180 e (73) 99833-4180. Só use no caso descrito em "Assunto da área médica".\n', ''),
    '## Os fatos que você pode informar', '## Os dois únicos fatos que você pode informar'), updated_at = now()
  where org_id = '2f61f310-1f2b-4049-9f2e-df9b6bc699e6' and md5(system_prompt) = '64d2d8fa9624151105c3d68788fc4d5f';
  ```
- **Próximo:** conferir as respostas reais da IA a assunto médico nas 24h seguintes (mesma busca por palavras-chave do levantamento, a partir de 17/09 12:49 UTC).

### 2026-09-17 · configuração · 🔴 Importação das 10h encerrou 98 orçamentos e os relatórios vieram pela metade

- **O que aconteceu (conferido no banco, só leitura):** duas importações hoje (09:49–09:50 e 10:00, horário da Bahia). A das 10:00 rodou com **"Encerrar como Não aprovado quem passou de 7 dias" MARCADA**: **98 orçamentos (73 pacientes, R$ 83.101)** foram de "Orçamento sob avaliação" para "Não aprovado" (`status='lost'`, `lost_reason='Sem resposta em 7 dias…'`), com nota "Encerrado por prazo". O gatilho `deals_stage_clock_bu` carimbou `stage_entered_at = 17/09 13:00 UTC` nos 98: a data do orçamento continua certa em `external_ref` (3ª parte) e no campo `dt_orcamento` (98/98 iguais), mas o filtro de período do disparo lê `stage_entered_at` → "Hoje"/"Ontem" passariam a pegar esses 98.
- **Legítimo na mesma importação:** 8 orçamentos foram para Aprovado (2 criados, 2 de Sob avaliação, 1 de Paciente marcado, 3 que já estavam lá).
- **Nada foi enviado:** 0 automações de funil, 0 regras de follow-up ativas, fila só com a "Teste" pausada.
- **🔴 Os arquivos do WebDental vêm cortados em 100 linhas.** A tela do relatório carrega por rolagem e o "Download XLS" leva só a primeira página. Não aprovados (01 a 16/09): cabeçalho diz **193 tratamentos / 148 pacientes / R$ 182.800,23**; o arquivo tem **100 linhas** (78 pacientes, R$ 83.990,75, só de 01 a 09/09). Aprovados: cabeçalho **167 / 140 / R$ 80.671,14**; arquivo **100 linhas** (R$ 51.626,25). O importador **não compara** as linhas lidas com o total do cabeçalho.
- **Status:** aguardando decisão do Danilo sobre a reversão dos 98 e o novo jeito de exportar.
- **Importação do dia 16/09 (10:13–10:14) e disparo das 10:16 — conferidos, CORRETOS.** Arquivos `-3` (não aprovados 16/09: 13 tratamentos / 12 pacientes / R$ 21.692,05) e `-4` (aprovados 16/09: 8 / 7 / R$ 3.701,06) vieram **completos** (linhas = cabeçalho). Entraram 13 em Sob avaliação e 3 aprovados novos; nenhum encerramento novo (continuam os 98 das 10:00). Campanha "16/09/2026" (`b01a216c`): funil inteiro + data 16/09 + trava 7 dias, template `odonto_orcamento_lembrete_d3` com {{1}} = nome do contato. Dos 12 pacientes do dia, **8 receberam, 8 entregues**; ficaram de fora com razão: 2 só com aprovado (Cat, Joelma), 3 pela trava (Bruna e Rita receberam 15–16/09; Domingas em 11/09). Letícia tem aprovado + outro em aberto → recebeu (decisão B). Donas: Millena 5, Larah 3. A "resposta" da Claudiane é mensagem automática de outro WhatsApp Business ("Dinha costureira agradece seu contato"), não resposta real. Observação: a variável leva o **nome completo** do cadastro.
- **Danilo questionou "eram mais de 8 pacientes" (17/09).** O arquivo `-3` tem **12 nomes**, mas **Maria Da Conceição Pinto e Artulita Claudemira Da Conceição usam o mesmo telefone** (+55 71 98522-1125). A importação identifica o paciente pelo telefone: os dois orçamentos de implante (R$ 4.582,32 e R$ 4.372,48) ficaram no cadastro da Artulita, e **não existe contato "Maria Da Conceição Pinto"**. Conta: 12 nomes → 11 cadastros → 3 barrados pela trava → **8 enviados**. A mensagem chegou ao número, mas saudando só a Artulita. Risco geral: família com telefone compartilhado vira um paciente só no CRM.

### 2026-09-17 · configuração · IA passa a parar de verdade depois de chamar a recepção

- **Pedido (Danilo, 17/09):** "resolve a IA parar de responder depois de chamar a recepção". Confirmado ("Confirma").
- **Causa:** o `process-ai-message` já pausa a IA quando a resposta traz `[HANDOFF]` **sozinho numa linha** (`ai_paused=true`, `status=human_active`; o gatilho `_on_handoff_notify` avisa a dona da conversa ou, sem dona, a equipe toda). As instruções nunca mandavam usar a marcação: só diziam "não responda mais", sem efeito mecânico. A atendente responder já pausa a IA (`send-operator-message`); o buraco era entre a promessa e a primeira resposta humana.
- **Tamanho (05 a 17/09, busca por "chamando/chamar/chamo a recepção" etc.):** 149 conversas em que a IA chamou a recepção; em 61 ela continuou respondendo (150 mensagens antes de humano); **23 continuam com IA ligada e nenhuma resposta humana depois da promessa** (a mais antiga de 08/09; 7 com Nathaly/Andressa, 2 sem dona; em todas a última fala é da IA).
- **Feito:** seção "Como encerrar" de `ai_agent_config.system_prompt` reescrita: frase de passagem + `[HANDOFF]` sozinho na última linha (com exemplo); usar sempre que chamar recepção/equipe (preço, tratamento, agenda, dor, áudio/foto, fora do horário, assunto que não é dela); **não** usar ao só cumprimentar, ao mandar o contato da área médica e ao perguntar se é odonto ou médico; nunca no meio da frase. Trava `md5 = 64d2d8fa…`. Aplicado 17/09 13:24 UTC → md5 `7f0882837eb0ebbc12b753c27c6d42dd`, 5869 caracteres. Nenhum código, nenhum deploy.
- **Reverter:** trocar o bloco que começa em `## Como encerrar` (vai até o fim do texto) pelo original:
  "## Como encerrar\n\nSempre termine sinalizando a passagem, com naturalidade:\n\"Já estou chamando a recepção, em instantes alguém fala com você por aqui.\"\n\nDepois disso, não responda mais — mesmo que ele escreva de novo." — o md5 deve voltar a `64d2d8fa9624151105c3d68788fc4d5f`.
- **Risco vigiado:** se a IA escrever `[HANDOFF]` no meio da frase, o `extractHandoff` não reconhece e a palavra vai para o paciente. Correção robusta seria no código (`extractHandoff` aceitar a marcação em qualquer posição), mas `process-ai-message/index.ts` tem diff pendente da frente WhatsApp/IA — não tocado.
- **As 23 conversas antigas:** a pergunta A (pausar agora, 23 avisos) × B (não mexer, entregar a lista) **não foi respondida**; nada foi alterado.
- **Próximo:** conferir as primeiras respostas reais depois de 13:24 UTC — pausou? vazou "[HANDOFF]"? usou quando não devia (cumprimento, área médica)? — e a decisão das 23.
- **Conferência CRM × WebDental de setembro (17/09, só leitura).** Arquivos `-5`/`-6` de novo cortados em 100 linhas (período 01–16/09 inteiro). Cabeçalhos: não aprovados **192 / 147 pac / R$ 180.643,71**; aprovados **167 / 140 / R$ 80.671,14**. CRM (por `dt_orcamento` e `won_at`): não aprovados **205 / 145 / R$ 195.980,46**; aprovados **168 / 136 / R$ 79.389,58**.
  - **Linha a linha (as 196 linhas que vieram, sem DENTALVIDAS): nada falta no CRM.** 99/99 não aprovados presentes (97 deles entre os 98 encerrados); 96/97 aprovados presentes. A exceção é Henrique Miguel (02/09), aprovado no WebDental **em 17/09** por R$ 167,44 — ainda não importado.
  - **🔴 Orçamento-fantasma = aprovado com valor renegociado.** Quando o paciente aprova por valor diferente, o WebDental tira a linha de não aprovados e lança o aprovado com o valor novo. A importação cria o aprovado como card NOVO e só põe nota de "sumiço" no antigo, que **continua aberto** → o paciente segue recebendo cobrança. Confirmados em 01–09/09 (ausentes do relatório de não aprovados, com nota de sumiço): **Maria Julia Ferreira Conceição** (R$ 433,86 → aprovado R$ 400 em 11/09), **Emanuele Caroline** (525,82 → 170 em 15/09), **Henrique Miguel** (2.156,52 → 167,44 em 17/09, está entre os 98), **Alineci Cardoso** (1.594,06 → 949,90 em 10/09), **Iremaldo Correia** (910,15 → 230,96 em 11/09), **Tania Santos Vieira** (1.047,38 → 299,75 em 14/09). A diferença de +13 no total aponta **~7 casos iguais em 10–16/09** (candidatos pelo par aberto+aprovado no mesmo paciente/dia/tratamento: Aline Neres, Adrizza Correia, Esheley Souza, Pietro De Andrade, João Batista ×2, Bernardo Lacerda) — **não confirmados**: mesmo paciente/dia/tratamento pode ter dois orçamentos legítimos (ex.: Gustavo e Laura Hage estão nos dois relatórios).
- **Extração direta do WebDental pelo Chrome (17/09, só leitura) — jeito que resolve o corte de 100 linhas.** Caminho: ícone de relatórios no `index.php` → formulário `relatorios/relatorios_paciente_tratamento.php` (campos `dtini`, `dtfim`, `exibir` = `NA|A|T`, `tipo` = `C|D`) → botão Gerar (`enviar_dados()`) abre `relatorios/relatorios_base.php` em aba nova **com TODAS as linhas no DOM** (sem paginação — a paginação só existe no XLS/página rolável). Leitura: `document.querySelectorAll('tr')` e, como o `javascript_tool` corta a saída em ~1.000 caracteres, escrever o resultado compacto num `<pre>` e ler com `get_page_text` (7 KB de uma vez). ⚠️ Na tela, a coluna **Valor da ortodontia é o contrato cheio** (ex.: R$ 2.880) e no XLS é a parcela (R$ 16,00) — casar orto por paciente+data+tratamento, nunca por valor.
- **Resultado do cruzamento 01–16/09 (WebDental 191 não aprovados / 162 linhas de aprovados × CRM):**
  - **NÃO APROVADOS: 191 de 191 no CRM. Nada falta** — a hipótese do Danilo ("as importações diárias já trouxeram tudo") está certa. O CRM abre **191 orçamentos** de setembro, R$ 179.044 (WebDental R$ 177.851; a diferença é o valor da orto).
  - **14 cards fantasmas** (aberto no CRM, ausente do WebDental = aprovado com valor renegociado), em 11 pacientes: Carlos Alberto Dorea (01/09, entre os 98), Maria Julia (01/09), Emanuele Caroline (02/09), Henrique Miguel (02/09, entre os 98), Alineci (03/09), Iremaldo (04/09), Tania (05/09), Adrizza Correia (4 cards, 10 e 11/09), Ruan Vitório (10/09), João Batista (2 cards, 14/09).
  - **Faltam no CRM 7 aprovados:** Henrique Miguel (02/09, aprovado hoje — entra na importação de hoje) e **6 de ortodontia** (Josué Celestino 04/09, Lindimara 04/09, Pedro Henrique Carvalho 04/09, Jefferson Nascimento 11/09, Macele Mendes 16/09, Marise dos Santos 16/09). Cinco desses pacientes **não existem no CRM** (nenhum card), então não recebem disparo; Marise tem 1 card aberto. Suspeita: linha de orto aprovada no mesmo dia nunca apareceu no relatório de não aprovados do dia, e o de aprovados baixado vinha cortado.
  - "A mais no CRM" em aprovados (20) são aprovações de agosto/valor zero fora do recorte do relatório — não são erro.
  - **Público do disparo do mês, já sem os fantasmas:** 137 pacientes com orçamento aberto de 01–16/09; 61 barrados pela trava de 7 dias; **76 recebem agora**.


### 2026-09-18 · configuração · Disparo do lote de 17/09 e o susto dos "84 pacientes"

- **Importação do dia (18/09 09:43–09:44), conferida e CORRETA.** Arquivos do WebDental com filtro `PERÍODO 17/09 a 17/09`: *APENAS NÃO APROVADOS* (19 linhas, 2 DENTALVIDAS) → **17 orçamentos / 14 pacientes / R$ 17.285,88** em "Orçamento sob avaliação"; *APENAS APROVADOS* (12 linhas, 1 DENTALVIDAS) → **11 aprovados**. Sem corte de 100 linhas.
- **🔴 O assistente ofereceu 84 pacientes no recorte de 17/09 — e o número estava "certo" pela regra atual.** Causa: o filtro de período lê `deals.stage_entered_at`, que para card **movido** é a data da movimentação, não a data do orçamento. Como em 17/09 **100 orçamentos antigos foram encerrados** (movidos para "Não aprovado", 75 pacientes), eles entraram no recorte do dia. Conta reproduzida em SQL: funil inteiro + 17/09 = **84** (14 do lote + 71 dos encerrados); só a coluna "Orçamento sob avaliação" + 17/09 = **14**. A trava de 7 dias não barra ninguém do lote e ninguém é "só aprovado".
  - ⚠️ Correção de um número que eu mesmo dei antes na sessão: estimei 6 encerrados no recorte usando `status='open'`; a regra do código conta **qualquer card não aprovado e não arquivado** (inclusive `lost`), logo são **71**. Ao conferir público, usar a regra do `useCampaigns.ts`, não uma aproximação.
- **Disparo feito pelo Danilo (18/09 12:54 UTC / 09:54 Itabuna), campanha "17/09/2026" (`3e2fd5b4`) — correto:** público = pipeline Odonto + `stage_ids` **só "Orçamento sob avaliação"** + `deal_from/deal_to = 2026-09-17` + `exclude_messaged_days = 7`; template **`odonto_orcamento_d1_v3`** (o que diz "você passou por avaliação ontem"), `{{1}}` = `contact_field.name`.
- **Resultado (fila encerrada em ~40s):** 14 na fila → **12 entregues** (1 já lida) · **2 falhas "Message undeliverable"**: **Gilnai Batista Dos Santos** (+5573981088831, R$ 3.790,80 — 1ª falha) e **José De Jesus Santos** (+5573988064522, R$ 1.768,52 — **2ª falha**, já tinha falhado em 09/09 na campanha "Agosto 2026"; número provavelmente sem WhatsApp). Os dois são o 2º e o 3º maiores valores do lote → **ligação da recepção**. Texto conferido no banco: saiu sem o defeito `1}}`, com **nome completo** ("Olá, Daniela Santos Barbosa!"). Donas das 14 conversas: Millena 9 · Larah 5 (trigger de 16/09 funcionando).
- **Pendência proposta, sem "pode" ainda:** fazer o filtro de período do disparo usar o campo **`dt_orcamento`** (data real do orçamento, já guardada em `custom_field_values`) em vez de `stage_entered_at`. Sem isso, todo dia com encerramento em massa repete o risco de misturar "orçado no dia" com "encerrado no dia". Risco baixo, mexe em `useCampaigns.ts`.
- **Também segue de pé:** a Edge Function `dispatch-campaign` publicada é a **v2**, sem a segunda trava do aprovado (`contatosSoComAprovado`) — o commit `2bedb10` está em `origin/main`, então o frontend já filtra; falta só o deploy da função como redundância.

### 2026-09-18 · configuração · Base de setembro unificada, template de captação escolhido

- **Pedido (Danilo):** extrair do WebDental todos os orçamentos aprovados e não aprovados (tipo Detalhado) para reimportar e fazer um disparo de captação.
- **Extração pela tela, sem o corte de 100 linhas (confirmada).** `POST relatorios_base.php` com o FormData do formulário `relatorios_paciente_tratamento.php` (`dtini`, `dtfim`, `exibir` NA|A|T, `tipo` C|D) devolve o relatório inteiro, e o `fetch` não renderiza a tabela — renderizar 2.500 linhas **congela a aba** (aconteceu duas vezes; o `enviar_dados()` submetendo na própria aba trava o renderer). Ano de 2026 inteiro: **2.554 não aprovados / 1.898 pacientes** e **2.757 aprovados / 1.801 pacientes**.
- **O HTML da TELA não é o mesmo formato do XLS** e o importador recusaria: a tela chama a coluna de valor de **"Valor"** (o parser exige "Valor Total"), traz **"Convênio"** no lugar de "Tabela do Orçamento" e tem **uma célula a mais no cabeçalho**, que desloca todas as colunas. Conversor em `scratchpad/converter_webdental.py`; arquivo do ano validado com o parser real (2.554 orçamentos, 1.865 pacientes, R$ 2.712.073,31, zero linha rejeitada).
- **Chrome bloqueia o 2º download automático do mesmo site** (o 1º passa). Clique real num `<a download>` criado na página não contorna — exige liberar na omnibox.
- **🔴 Achado que muda o procedimento diário: exportar com "Exibir: TODOS" trunca em 100 linhas e o CRM não avisa sozinho na conta.** Dos arquivos do Danilo, 2 de 3 vieram cortados (127→100 e 139→100). A conferência correta é **o número do topo ("QTD Total de Tratamento…") contra a quantidade de linhas**. Separando por APENAS NÃO APROVADOS / APENAS APROVADOS, cada fatia cabe.
- **🔴 Faixas de período que se sobrepõem geram CARDS DUPLICADOS.** As faixas 11–14/09 e 14–16/09 repetiam o dia 14: 55 linhas iguais. O `combinarLeituras` **não** deduplica — trata linha repetida como "dente diferente" e numera 1º/2º, criando card novo. Subir os 5 arquivos daria **416 orçamentos em vez de 362** (dia 14 com 54 em vez de 27). Unificador em `scratchpad/unificar.py`, com a regra certa: **o máximo observado num único arquivo**, nunca a soma (linha repetida dentro do mesmo arquivo é legítima e é preservada).
- **Entregue:** `~/Downloads/SETEMBRO_01-16_UNIFICADO.xls` — 370 linhas, validado pelo parser do CRM: **362 orçamentos · 262 pacientes · 172 aprovados (R$ 84.839,38) · 190 em aberto (R$ 175.588,29) · 8 linhas DentalVidas fora · zero rejeitada**. Dias 06, 07 e 13/09 sem movimento (domingos + feriado da Independência).
- **⚠️ Na importação, as duas caixas têm de ser DESMARCADAS** (ambas vêm ligadas): "encerrar quem passou de 7 dias" alcançaria **318** orçamentos parados; "sinalizar sumiço" usaria a janela derivada do arquivo e marcaria ~180 dos 384 abertos do CRM sem motivo.
- **Template escolhido para quem não aprovou: `odonto_condicao_este_mes`** — único testado em base fria (217 envios em 09/09 para a base de agosto): **190 entregues, 65 respostas, 34,2%**. Comparação: `odonto_orcamento_d1_v3` 39,0% mas é base quente e diz "avaliação **ontem**"; `odonto_orcamento_d1` 24,6%; `lembrete_d3` só 8 envios. Descartados: `odonto_orcamento_validade` (promete prazo de 7 dias que já venceu e "tabela nova"), `ultima_chamada` (fala em "uma semana" e oferece "Não quero mais" no 1º toque), `condicoes_especiais` (sem motivo de agir e sem histórico). **`lembrete_d3` fica para o 2º toque.**
- **Público medido (18/09):** 145 pacientes com orçamento de 01–16/09 não aprovado (77 já encerrados por prazo); 96 passam pela trava de 7 dias. Pelo filtro que a tela permite (colunas "Sob avaliação" + "Não aprovado", período 01–18/09): 163 candidatos → 59 barrados → **104 recebem**.
- **⚠️ O botão "disparar lote" da tela de importação NÃO serve para esta campanha:** `contatosParaDisparo` só junta contatos de deals **criados** naquela importação. Como setembro já está no CRM, o lote novo é pequeno. A campanha de captação tem de ser montada em Disparos, pelo filtro.
- **Não feito:** importação e campanha **não foram executadas** — a extensão Claude in Chrome caiu no meio (`not connected`) e a sessão do CRM é a do navegador do Danilo. Passo a passo entregue a ele.
- **Pendência de negócio:** a mensagem promete "condições especiais" — a recepção precisa da condição concreta antes do disparo (a decisão sobre desconto/bonificação está aberta desde 15/09).

### 2026-09-18 · configuração · Público "Quem não aprovou" com recorte pela data do orçamento (código pronto, NÃO publicado)

- **Pedido (Danilo, 18/09):** "preciso enviar o disparo apenas para pacientes não aprovados, quero que você crie esse filtro — onde eu seleciono pacientes não aprovados e a data".
- **Por que não dava para fazer pela tela:** (1) "não aprovado" está espalhado por duas colunas — os que seguem em "Orçamento sob avaliação" e os **encerrados por prazo**, que viram a coluna "Não aprovado" e somem de qualquer recorte de etapa; (2) o filtro de período olhava `deals.stage_entered_at`, a data de entrada na coluna ATUAL — para card movido, encerrado ou aprovado essa data é a da movimentação. Foi o que fez "17/09" alcançar 84 pacientes em vez de 14 nesta mesma manhã.
- **Feito (não publicado):**
  - `src/types/campaigns.ts`: `AudienceFilter` ganhou `nao_aprovados`, `orcamento_from`, `orcamento_to` — documentados como distintos de `deal_from`/`deal_to`.
  - `src/hooks/useCampaigns.ts` (⚠️ CRLF, preservado): `diaSeguinte`, `idDoCampoDataDoOrcamento`, `orcamentosNoPeriodo` (lê `custom_field_values` do campo `dt_orcamento` em páginas de 1000, com `lt(dia seguinte)` para tolerar valor com hora) e `contatosQueNaoAprovaram`. `OrcamentoLinha` passou a carregar `id`. O novo público entra em `resolveAudienceIds` antes das outras interseções e também no portão de segurança (filtro vazio continua resolvendo para zero, nunca para todos).
  - `src/components/campaigns/CampaignWizard.tsx`: modo **"Quem não aprovou"** como primeira opção e padrão, com seletor **"Data do orçamento"** (Qualquer data · Hoje · Ontem · Últimos 7 dias · **Este mês** · Escolher datas) e texto explicando a diferença entre a data do orçamento e a da coluna.
- **🔴 Bug achado de passagem e corrigido:** o `useEffect` de abertura do assistente fazia `setAudienceMode('all')`, então **toda vez que a tela abria ela caía em "Todos os pacientes"** — desfazendo na prática a decisão de 08/09/2026 de não abrir apontando para a base inteira (o `useState` era 'funnel', mas o reset vencia). Agora abre em "Quem não aprovou" e zera o período.
