...
| configuração | 16/09 09:00 | migration `20260916120000_conversa_nasce_com_dona.sql` (trigger BEFORE INSERT em `conversations` + coluna `app_users.atende_inbox`) | **concluído** — aplicada com o "pode" do Danilo. Só Millena e Larah recebem. ⚠️ **Frente da transferência de conversa parada:** a coluna `atende_inbox` já existe e o passo 4 da `20260909180000` (marca 4 atendentes) precisa virar só Millena e Larah. Acerto do acumulado (13 conversas) bloqueado por permissão, aguarda o Danilo |
| configuração | 16/09 10:00 | `src/hooks/useCampaigns.ts` · `src/components/campaigns/CampaignWizard.tsx` · `src/components/funil/ImportOrcamentosDialog.tsx` (só o aviso do toast) · `supabase/functions/dispatch-campaign/index.ts` | **concluído, NÃO publicado** — disparo não vai para paciente que só tem orçamento aprovado (decisão B do Danilo: quem tem outro orçamento em aberto continua recebendo). Edge Function e frontend aguardam o "pode" |
| configuração | 17/09 10:00 | `src/app/routes/dashboard/DashboardPage.tsx` · `src/lib/dashboard.ts` (só o período padrão e o rótulo do período; nenhuma cor ou estrutura) | **concluído, NÃO publicado** — Painel abre sempre no mês corrente (opção A do Danilo: todos os quadros seguem o mês). Aguarda o "pode" |
