...
| configuração | 17/09 10:00 | `src/app/routes/dashboard/DashboardPage.tsx` · `src/lib/dashboard.ts` (só o período padrão e o rótulo do período; nenhuma cor ou estrutura) | **concluído** — publicado em `1a5aecf`. Painel abre sempre no mês corrente (opção A do Danilo: todos os quadros seguem o mês) |
