...
const PALETTE = ['#0A7787', '#0B6E7D', '#0C6B4A', '#9A4A07', '#6D28D9', '#B02D26', '#4E666B'];
        ...
        {data.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} layout="vertical" margin={{ top: 4, right: 12, left: 90, bottom: 0 }}>
              <XAxis type="number" tick={{ fontSize: 11, fill: '#4E666B' }} stroke="rgba(97,193,208,0.45)" tickFormatter={(v) => brl(Number(v))} />
              <YAxis type="category" dataKey="name" width={90} tick={{ fontSize: 11, fill: '#5C7378' }} stroke="rgba(97,193,208,0.45)" />
              <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v) => [brl(Number(v)), 'Ganho']} />
              <Bar dataKey="value" fill="#0C6B4A" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
    ...
    <WidgetCard
      ...
      titleExtra={
        <span className="group relative mr-auto flex items-center">
          ...
          <span className="pointer-events-none absolute left-0 top-full z-20 mt-1.5 w-60 rounded-lg border border-[rgba(97,193,208,0.55)] bg-[#FFFFFF] px-3 py-2 text-xs font-normal normal-case tracking-normal text-[var(--color-text-primary)] opacity-0 shadow-[0_4px_14px_rgba(23,40,43,0.08)] transition-opacity group-hover:opacity-100">
        ...
        {rows.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={rows} layout="vertical" margin={{ top: 4, right: 36, left: 100, bottom: 0 }}>
              ...
              <YAxis type="category" dataKey="name" width={100} tick={{ fontSize: 11, fill: '#5C7378' }} stroke="rgba(97,193,208,0.45)" />
