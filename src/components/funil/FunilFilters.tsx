...
function MultiSelect({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute z-30 mt-1 max-h-56 w-full overflow-y-auto rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-bg-surface)] p-1 shadow-[0_6px_20px_rgba(23,40,43,0.10)]">
          {options.length === 0 && (
            <div className="px-2 py-1.5 text-xs text-[var(--color-text-secondary)]">Nenhuma opção</div>
          )}
          {options.map((o) => (
            <label key={o.id} className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-sm text-[var(--color-text-primary)] hover:bg-[var(--color-bg-subtle)]">
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-2">
        <button
          onClick={() => setOpen((v) => !v)}
          className="inline-flex items-center gap-2 rounded-lg border border-[var(--color-border-strong)] px-3 py-2 text-sm text-[var(--color-text-primary)] transition hover:border-[var(--accent-primary)]"
        >
        ...
        <label className="inline-flex items-center gap-2 rounded-lg border border-[var(--color-border-strong)] px-3 py-2 text-sm text-[var(--color-text-primary)] transition hover:border-[var(--accent-primary)]">
          <ArrowUpDown className="h-4 w-4 opacity-70" />
          <select
            value={sort}
            onChange={(e) => onSortChange(e.target.value as FunilSort)}
            className="bg-transparent text-sm text-[var(--color-text-primary)] outline-none [&>option]:bg-[var(--color-bg-surface)]"
          >
