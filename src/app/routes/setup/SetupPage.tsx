  ...
  return (
    <button
      ...
      className={[
        ...
        'bg-[linear-gradient(135deg,#0B6E7D_0%,#0A7787_100%)] shadow-[0_6px_18px_rgba(11,110,125,0.22)]',
        'hover:shadow-[0_6px_18px_rgba(11,110,125,0.22)] disabled:cursor-not-allowed disabled:opacity-40 disabled:shadow-none',
        'w-full sm:w-auto',
        props.className ?? '',
      ].join(' ')}
    />
        ...
        return (
          <div key={label} className="flex min-w-0 flex-1 items-start last:flex-none sm:flex-none">
            <div className="flex flex-col items-center">
              <div
                className={[
                  ...
                  active
                    ? 'bg-[#0A7787] text-white shadow-[0_4px_14px_rgba(23,40,43,0.08)]'
                    : complete
                      ? 'bg-[#0B6E7D] text-white'
                      : 'border border-[rgba(97,193,208,0.70)] bg-transparent text-[#4E666B]',
                ].join(' ')}
              >
              ...
              <div
                className={[
                  ...
                  active ? 'text-[#17282B]' : 'text-[#4E666B]',
                ].join(' ')}
              >
                {label}
              </div>
            ...
            {index < STEP_LABELS.length - 1 ? (
              <div className="mx-2 mt-5 h-px w-8 border-t border-[rgba(97,193,208,0.45)] sm:mx-5 sm:w-20" />
            ) : null}
          </div>
        );
      })}
...
function SetupCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-[rgba(97,193,208,0.38)] bg-[#FAFDFD] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-[40px] md:p-12">
      {children}
    </div>
  );
}
...
function PrepItem({
  ...
  return (
    <div className="relative rounded-xl border border-[rgba(97,193,208,0.30)] bg-[#FAFDFD] p-5">
      <div className="flex gap-4 pr-16">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[rgba(11,110,125,0.40)] text-sm font-medium text-[#0B6E7D]">
          {n}
        </div>
        <div>
          <h2 className="text-base font-semibold text-[#17282B]">{title}</h2>
          <p className="mt-1 text-[13px] leading-5 text-[#4E666B]">{text}</p>
          <div className="mt-4 flex flex-wrap gap-2">
            {pills.map((pill) => (
              <span
                key={pill}
                className="rounded-full border border-[rgba(97,193,208,0.70)] bg-[rgba(30,58,138,0.4)] px-3 py-1 font-mono text-[11px] uppercase tracking-[0.05em] text-[#0B6E7D]"
              >
                {pill}
              </span>
            ))}
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="absolute right-5 top-5 inline-flex items-center gap-1 text-sm text-[#0B6E7D] hover:text-[#0B6E7D]"
      >
        abrir
...
export default function SetupPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] px-4 py-8 text-[#17282B] md:py-12">
      <div className="mx-auto w-full max-w-[760px]">
        <StepIndicator step={step} />
        <SetupCard>
          {step === 1 ? (
            <>
              <h1 className="mb-2 text-[28px] font-semibold text-[#17282B]">{setupConfig.toolName}</h1>
              <p className="mb-8 text-base leading-[1.6] text-[#4E666B]">
          ...
          {step === 2 ? (
            <>
              <h1 className="mb-2 text-[28px] font-semibold text-[#17282B]">Credenciais core</h1>
              <p className="mb-8 text-base leading-[1.6] text-[#4E666B]">
              ...
              <div className="grid gap-4">
                {(Object.keys(emptyCore) as FieldKey[]).map((key) => {
                  ...
                  return (
                    <div key={key}>
                      <label className="mb-1.5 block text-[13px] font-medium text-[#5C7378]">
                        {key.toUpperCase()}
                      </label>
                      <div className="relative">
                        <input
                          ...
                          className="w-full rounded-lg border border-[rgba(97,193,208,0.45)] bg-[#F7FBFC] px-4 py-3 pr-16 text-sm text-[#17282B] placeholder:text-[#4E666B] focus:border-[#0A7787] focus:outline-none focus:shadow-[0_4px_14px_rgba(23,40,43,0.08)]"
                        />
                        <div className="absolute right-3 top-1/2 flex -translate-y-1/2 items-center gap-2">
                          ...
                          {validation[key]?.ok ? <Check className="h-4 w-4 text-[#0C6B4A]" /> : validation[key] ? <X className="h-4 w-4 text-[#B02D26]" /> : null}
                        </div>
                      </div>
                      {validation[key]?.message ? (
                        <p className={validation[key]?.ok ? 'mt-1 text-xs text-[#0C6B4A]' : 'mt-1 text-xs text-[#B02D26]'}>
                          {validation[key]?.message}
                        </p>
                      ) : null}
                    </div>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="min-h-11 w-full rounded-lg border border-[rgba(97,193,208,0.55)] bg-[#F7FBFC] px-5 text-sm font-medium text-[#17282B] transition hover:border-[#0A7787] sm:w-auto"
                >
                  Voltar
                </button>
          ...
          {step === 3 ? (
            <>
              <h1 className="mb-2 text-[28px] font-semibold text-[#17282B]">Setup</h1>
              <p className="mb-8 text-base leading-[1.6] text-[#4E666B]">
                Preparando Supabase, Edge Functions, owner e Vercel.
              </p>
              <div className="space-y-3">
                {TIMELINE_STEPS.map((entry) => (
                  <div key={entry.label} className="flex items-center gap-3 rounded-xl border border-[rgba(97,193,208,0.30)] bg-[#FAFDFD] p-4">
                    {timeline.includes(entry.label) ? <Check className="h-5 w-5 text-[#0C6B4A]" /> : <Loader2 className="h-5 w-5 animate-spin text-[#0B6E7D]" />}
                    <span className="text-sm text-[#17282B]">{entry.label}</span>
                  </div>
                ))}
              </div>
              {waitingForApp ? (
                <p className="mt-6 text-center text-sm text-[#4E666B]">
              ...
              {setupDone ? (
                <div className="mt-8 rounded-xl border border-[rgba(16,185,129,0.3)] bg-[rgba(16,185,129,0.06)] p-5">
                  <p className="text-sm leading-5 text-[#17282B]">
                ...
                <div className="mt-8 rounded-xl border border-[rgba(239,68,68,0.3)] bg-[rgba(239,68,68,0.06)] p-5">
                  <p className="text-sm leading-5 text-[#17282B]">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-[#0B6E7D] underline hover:text-[#0B6E7D]"
                    >
                  ...
                  <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:justify-end">
                    <button
                      ...
                      className="min-h-11 rounded-lg border border-[rgba(97,193,208,0.55)] bg-[#F7FBFC] px-5 text-sm font-medium text-[#17282B] transition hover:border-[#0A7787]"
                    >
                      Verificar de novo
                    </button>
                  ...
                  {!core.owner_password ? (
                    <div>
                      ...
                      <input
                        ...
                        className="w-full rounded-lg border border-[rgba(97,193,208,0.45)] bg-[#F7FBFC] px-4 py-3 text-sm text-[#17282B] placeholder:text-[#4E666B] focus:border-[#0A7787] focus:outline-none focus:shadow-[0_4px_14px_rgba(23,40,43,0.08)]"
                      />
                    </div>
                  ) : null}
