      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          {DAY_ORDER.map(({ key, label }) => (
            <div
              key={key}
              className="grid grid-cols-[auto_100px_1fr_1fr] items-center gap-3 p-3 rounded-lg border border-[rgba(97,193,208,0.30)] bg-[#FAFDFD]"
            >
              <input
                type="checkbox"
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            rows={3}
            disabled={saving}
            className="w-full rounded-lg border border-[rgba(97,193,208,0.45)] bg-[#F7FBFC] px-4 py-3 text-sm text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--accent-primary)] focus:bg-[#EEF6F7]"
          />
        </div>
